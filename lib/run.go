...
func RunTask(profile, cluster, service, taskDefinitionName, imageTag string, imageTags []string, workDir, containerName, awslogGroup, launchType string, args []string) (exitCode int, err error) {
    ...
    log.Infof("Session created successfully")

    svc := ecs.New(localSession)

    log.Infof("Describing task definition: %s", taskDefinitionName)
    ...
    log.Infof("Registering modified task definition")
    ...
    log.Infof("Task definition registered successfully")
    ...
    log.Infof("Running task in cluster: %s", cluster)
    ...
    log.Infof("Task started, waiting for it to finish")
